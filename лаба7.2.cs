...
abstract class DivingEvent
{
    public abstract string DisciplineName { get; }

    protected List<JumpResult> competitionResults;

    protected DivingEvent(List<JumpResult> results)
    {
...
class DivingEvent3m : DivingEvent
{
    public override string DisciplineName => "Прыжки с 3 метровой вышки";

    public DivingEvent3m(List<JumpResult> results) : base(results)
    {
    }
}

class DivingEvent5m : DivingEvent
{
    public override string DisciplineName => "Прыжки с 5 метровой вышки";

    public DivingEvent5m(List<JumpResult> results) : base(results)
    {
    }
}
...
class JumpCompetition
{
    static void Main()
    {
        ...
        var divingEvent3m = new DivingEvent3m(competitionResults);
        divingEvent3m.GenerateProtocol();

        var divingEvent5m = new DivingEvent5m(competitionResults);
        divingEvent5m.GenerateProtocol();
    }
}
